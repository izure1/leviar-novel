...
**Kotonoha**은 웹 기반의 강력하고 유연한 모듈형 비주얼 노벨 엔진입니다.

## 💡 왜 Kotonoha인가?

기존의 웹 기반 노벨 엔진들은 DOM 조작의 한계로 인해 복잡한 카메라 연출이나
고성능 파티클 효과를 구현하는 데 성능적 제약이 있었습니다. `kotonoha`은 **Canvas
기반의 고성능 렌더러**와 **반응형 상태 관리(MVC)**를 결합하여, 웹 환경에서도
고품질 게임에 버금가는 입체적인 연출을 쉽고 직관적인 코드로 구현하기 위해
탄생했습니다.

[데모 플레이](https://izure1.github.io/kotonoha/)
...
```bash
npm install kotonoha
```

### 2. 최소 구동 예제

```typescript
import { Novel } from "kotonoha";
