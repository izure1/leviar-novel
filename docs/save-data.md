...
| 수집 대상 | 상세 설명 |
| :--- | :--- |
| **진행 위치** | 현재 어느 씬(`Scene`)의 어느 명령어 스텝 위치에 멈춰있는지에 대한 포인터 |
| **변수 상태** | 게임 진행 중 `set`으로 조작한 모든 전역 변수 및 현재 씬의 활성 지역 변수 데이터 |
| **시각 및 청각 상태** | 화면에 떠 있는 오버레이, 배경, 캐릭터, 재생 중인 BGM과 현재 재생 시간 |
| **Call 스택** | `call` 예약어로 서브루틴에 진입했을 경우 부모 씬으로 되돌아가기 위해 기록된 콜 스택 |
...
세이브 파일은 최종적으로 JSON 텍스트 형태로 변환되어 기기에 보존됩니다. 따라서 엔진 변수에는 반드시 아래와 같은 순수한 직렬화 데이터만 담아야 합니다.  
