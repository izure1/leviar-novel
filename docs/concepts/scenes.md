...
문자열을 직접 지정하여 해당 씬으로 이동할 수도 있고, `{ scene: string, preserve: boolean }` 형태의 객체를 전달할 수 있습니다. `preserve: true`로 설정할 경우 배경, 캐릭터, 파티클 등 기존 렌더러 오브젝트들을 지우지 않고 상태를 유지한 채 부드럽게 씬을 전환할 수 있습니다. 

> [!IMPORTANT]
> **오디오 및 상태의 보존**: 
> * **오디오(Audio)**: 렌더러 오브젝트와 달리, 재생 중인 오디오는 `preserve: false` 상태로 씬을 전환하더라도 **자동으로 중단되지 않습니다.** 오디오는 모든 씬에서 공유되는 전역 요소로 취급되므로, 전환 시 음악을 멈추고 싶다면 명시적으로 `audio:stop` 커맨드를 사용해야 합니다.
> * **지역 변수**: `preserve: true`를 사용하더라도 **지역 변수(`variables`)는 유지되지 않고 새 씬의 초깃값으로 초기화됩니다.** 시각적인 연속성만 보존될 뿐, 로직상의 상태(변수)를 유지해야 한다면 반드시 전역 변수를 사용해야 합니다.
