...
이전 단계인 [08. 고급 설계 가이드](./08-advanced-guide.md)에서는 캐릭터 정의와 훅 시스템을 다루었습니다. 이번 장에서는 시스템 UI 구현의 핵심인 중첩 씬 호출에 대해 알아봅니다.
...
* **`preserve: false`**: 완전히 백지상태(초기화)에서 서브씬을 시작합니다. 기존 화면 자원(배경, 캐릭터 등)은 모두 지워지며 서브씬의 `initial`만 새롭게 적용됩니다. **단, 오디오는 전역 요소이므로 지워지지 않고 계속 재생됩니다.**
...
* **오디오 자동 동기화**: 서브씬 내에서 음악을 바꾸거나 멈추었더라도 복구 모드(`restore: true`)라면 안심하세요. 서브씬 진입 전의 원래 곡으로 되돌아가며, 호출 시점의 재생 시간을 기억해 그 시간대부터 **자동으로 다시 이어 재생**됩니다. (만약 서브씬과 호출자 씬이 **동일한 곡**을 재생 중이라면 되감기 없이 자연스럽게 이어서 재생됩니다.)
