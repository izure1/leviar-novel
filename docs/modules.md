...
엔진 코어가 특정 모듈 이름에 직접 의존하지 않도록, View는 반환 시 자신의 역할과 입력 처리 방식을 선언해야 합니다.

| 선언 속성 | 타입 | 역할 및 동작 원리 |
| :--- | :--- | :--- |
| **`uiGroup`** | `string` | 이 UI의 소속 그룹. (예: `'dialogue'`) |
| **`hideGroups`** | `string[]` | 이 UI가 활성화될 때 자동으로 숨겨야 할 그룹 목록. 엔진이 해당 그룹 UI의 `hide()`를 호출합니다. |
| **`inputSteps`** | `Record<string, boolean>` | 스텝 별 사용자 입력 진행 허용 여부. `false`인 경우 입력으로 인한 씬 진행이 차단됩니다. |
