...
본 문서는 `audio` 모듈을 사용하여 게임 내 사운드를 정교하게 제어하는 방법을 기술합니다.  

---

## 1. 개요 (Overview)

`audio` 모듈은 게임 내 배경음(BGM)과 효과음(SFX)의 재생 및 관리를 담당합니다.  
단순 재생을 넘어 트랙 간 **자동 크로스페이드(Crossfade)**와 페이드 인/아웃, 구간 반복 등 전문적인 오디오 연출 기능을 제공합니다.  
...
*   **상태 영속성**: 재생 중인 모든 오디오의 상태(곡명, 볼륨, 시간 등)는 엔진 상태에 보존되어 세이브/로드 시 완벽하게 복원됩니다.  
...
`audio` 커맨드는 `action` 속성에 따라 다양한 사운드 제어 기능을 수행합니다.  
...
오디오의 재생 상태를 추적하거나 특정 시점에 이벤트를 처리할 수 있습니다.  
[상세 가이드: Audio Hooks (오디오 훅)](./hooks/audio.md) 문서를 참조해 주십시오.  

---

## 5. 주의 사항 (Edge Cases)

*   **브라우저 자동재생 정책**: 사용자의 상호작용(클릭) 이전에 재생을 시도하면 차단될 수 있습니다.  엔진은 사용자의 상호작용이 발생하는 즉시 자동으로 재생을 재개합니다.  
*   **전역 공통 요소**: 오디오는 씬 전환 시에도 **자동으로 정지되지 않습니다.** `preserve: false`로 씬을 전환하더라도 재생 상태가 유지됩니다.  음악을 바꾸거나 멈추고 싶다면 새로운 씬에서 명시적으로 `stop` 커맨드를 사용해야 합니다.  
*   **리소스 관리**: `stop` 액션을 사용하면 해당 오디오 객체는 내부 풀(Pool)에서 제거됩니다.  
*   **명령어 중복 호출**: 동일 트랙에 연속적인 `play` 요청 시 마지막 요청의 페이드 효과가 우선권을 가집니다.  
