...
* [상세 가이드: Audio Hooks (오디오 훅)](./hooks/audio.md)
...
* **전역 공통 요소**: 오디오는 씬 전환 시에도 **자동으로 정지되지 않습니다.** `preserve: false`로 씬을 전환하더라도 재생 상태가 유지되므로, 음악을 바꾸거나 멈추고 싶다면 새로운 씬에서 명시적으로 `stop` 커맨드를 사용하거나 다른 곡을 `play`해야 합니다.
* **메모리 관리**: `stop` 액션을 사용하면 해당 오디오 객체는 풀(Pool)에서 제거됩니다. 
