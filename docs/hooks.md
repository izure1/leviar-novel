...
각 모듈(dialogue, choice 등) 고유의 동작과 관련된 훅입니다. 접두사로 모듈의 키를 사용합니다.

*   **`dialogue:text`**: 대화 텍스트가 출력되기 직전에 발생합니다. 출력될 텍스트나 화자 이름을 수정할 수 있습니다.
*   **`choice:show`**: 선택지가 화면에 표시되기 직전에 발생합니다.
