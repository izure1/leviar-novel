...
이제 여러분은 화면에 요소를 띄우고, 상태를 관리하며, 사용자 UI를 구성하는 핵심 기술을 모두 익혔습니다.

하지만 실제 게임 개발에서는 코드를 더 깔끔하게 관리하는 **설계(Architecture)**가 중요합니다.
다음 장에서 `scene-ui.ts` 예제와 같은 고급 관리 기법을 알아보세요.

**[7. 효율적인 씬 설계: 부모-자식 구조](./07-advanced-architecture.md)**
