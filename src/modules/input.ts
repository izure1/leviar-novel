...
export interface InputHook {
  'input:open': (value: { label: string; multiline: boolean }, ctx: SceneContext) => { label: string; multiline: boolean }
  /**
   * 입력 완료/취소 시 방출됩니다.
   * - `cancelled: true`이면 취소 버튼이 클릭된 것입니다.
   * - cancelled인 경우 `text`는 빈 문자열이며, varName에 저장되지 않습니다.
   */
  'input:submit': (value: { varName: string; text: string; buttonIndex: number; cancelled: boolean }, ctx: SceneContext) => { varName: string; text: string; buttonIndex: number; cancelled: boolean }
}
